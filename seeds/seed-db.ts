...
// --- Validation Schemas (Infrastructure/Domain compliance) ---
...
async function seedDB() {
	try {
		console.log('🧹 Cleaning up existing data (Order-safe)...')
		// Tables to clear in specific order to avoid FK errors
		const tables = [
		...
		console.log('👤 Seeding Users (Admin + Staff + Clients)...')
		...
		// 2. Staff (Therapists)
		const staffRoles = ['Alicia', 'Beatriz', 'Carolina']
		...
		// 3. Clients (Massive seed for list performance testing)
		for (let i = 1; i <= 60; i++) {
			...
			usersData.push(UserSchema.parse({
				...
				role: 'CLIENT',
				status: getRandomItem(['ON', 'ON', 'ON', 'OFF']), // Mostly active
		...
		for (let i = 0; i < 25; i++) {
			...
			await prisma.product.create({
				data: {
					...
					description: 'Fórmula enriquecida con activos botánicos para resultados visibles desde la primera aplicación.',
		...
		await prisma.coupon.createMany({
			data: [
				...
				{ code: 'VIP-GIFT', discount_type: 'percentage', discount_value: 20, description: 'Solo para clientes recurrentes' }
			]
		})

		// Relational CRM seeding (One-by-one to ensure quality)
			...
			if (Math.random() > 0.3) {
				await prisma.consent.create({
					data: {
						...
						notes: 'Firmado mediante firma digital biométrica.'
					}
				}).catch(() => {})
			}
				...
				await prisma.questionnaire.create({
					data: {
						...
						data: {
							...
							habits: ['Fumador', 'Deporte regular', 'Poca agua'],
							observations: 'Puntualizar tratamiento en contorno de ojos.'
						}
					}
				})
			}

			// Bookings (Past & Future appointments)
		...
		console.log('✅ Database seeded with premium data!')
		console.log(`🔑 Credentials: admin@beilsbellezahonesta.com / password123`)
