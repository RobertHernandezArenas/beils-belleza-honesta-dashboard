	...
	watch(
		...
		newVal => {
			...
			if (newVal) {
				initForm()
				nextTick(() => {
					animateOpen(brandDialog.value, { staggerChildren: true })
				})
			} else if (brandDialog.value?.open) {
				animateClose(brandDialog.value)
			}
		},
	)
...
<template>
	<dialog ref="brandDialog" class="modal modal-bottom sm:modal-middle" :class="{ 'modal-open': localVisible }">
