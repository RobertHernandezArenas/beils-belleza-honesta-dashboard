...
// Guardar o firmar consentimiento Láser
const { mutate: saveConsent, isPending: isSaving } = useMutation({
	...
	onError: (err: any) => {
		error.value = err?.data?.statusMessage || err.message || 'Error al guardar el consentimiento de Láser'
	}
})

// Revocar consentimiento formalmente
const { mutate: revokeConsent, isPending: isRevoking } = useMutation({
	mutationFn: async () => {
		...
		return await $fetch('/api/clients/revokes', {
			...
			body: {
				user_id: props.client.user_id,
				reason: `Revocación consentimiento LÁSER SHR: ${revokeReason.value || 'A petición del cliente'}`,
...
const handleWhatsApp = () => {
	...
	const message = `Hola ${clientName}, te adjuntamos la confirmación de tu consentimiento para Láser SHR Innova Pro en Beils Belleza Honesta. Recordá respetar las 72 horas sin exposición solar antes y después de cada sesión.`
...
const handleEmail = () => {
	...
	const clientName = `${props.client.name} ${props.client.surname || ''}`.trim()
	const subject = encodeURIComponent('Consentimiento Informado: Láser SHR - Beils')
	const body = encodeURIComponent(
		`Estimado/a ${clientName},\n\nLe confirmamos el registro y firma de su consentimiento para el tratamiento de depilación con Láser SHR en Beils Belleza Honesta.\n\nAtentamente,\nBeils Belleza Honesta`
	)
				...
				<div class="flex items-center gap-3">
					...
					<div>
						<h3 class="text-text-primary text-base sm:text-lg font-black tracking-tight flex items-center gap-2">
							Consentimiento: Láser Diodo SHR (Innova Pro)
							<span
						...
						<p class="text-text-muted text-xs font-semibold">Fotodepilación Médica / Estética por Diodo SHR</p>
					</div>
				</div>
			...
			<div class="p-4 sm:p-6 overflow-y-auto flex-1 space-y-6">
				...
				<div class="p-3 bg-bg-muted/20 border border-border-subtle rounded-2xl flex flex-wrap items-center justify-between gap-2 text-xs">
					...
					<div class="flex items-center gap-1.5">
						<button
							...
							@click="$emit('open-dossier', 'LASER_SHR')"
						>
				...
				<div v-if="isRevokeMode" class="p-5 bg-error/10 border border-error/30 rounded-2xl space-y-4">
					<div class="flex items-center gap-2 text-error font-black text-sm uppercase">
						<Ban class="w-5 h-5" />
						<span>Revocación de Consentimiento para Depilación Láser</span>
					</div>
					<p class="text-xs text-text-muted">
						El cliente manifiesta revocar su autorización para el tratamiento de fotodepilación con Láser SHR Innova Pro.
					</p>
					<div class="space-y-1.5">
						...
						<textarea
							...
							rows="2"
							placeholder="Ej: Finalización de sesiones, motivo personal..."
				...
				<template v-else>
					...
					<!-- 2. CLÁUSULAS OFICIALES INNOVA PRO (Página 2 del PDF) -->
					<div class="p-4 bg-bg-muted/15 border border-border-subtle rounded-2xl space-y-3 text-xs text-text-muted leading-relaxed">
						<span class="text-xs font-black uppercase tracking-wider text-text-primary block">
							Condiciones y Compromisos del Paciente
						</span>
						<ul class="space-y-2 list-disc list-inside text-[11px]">
							<li>
								<strong>Evolución y duración:</strong> Comprendo que la depilación láser actúa selectivamente sobre los folículos en fase anágena, siendo necesarias varias sesiones espaciadas para una reducción óptima.
							</li>
							<li>
								<strong>Protección Ocular:</strong> Es obligatorio portar las gafas de seguridad protectoras homologadas durante todo el disparo de la aparatología.
							</li>
							<li>
								<strong>Exposición Solar (Crítica):</strong> Me comprometo a NO exponerme al sol directo ni a rayos UVA al menos <span class="text-error font-bold">72 horas antes y después</span> de cada sesión.
							</li>
							<li>
								<strong>Método de depilación:</strong> Entre sesiones se prohíbe el arranque de raíz (cera, pinzas, máquinas depiladoras), permitiéndose únicamente el rasurado con cuchilla.
							</li>
							<li>
								<strong>Efectos esperados:</strong> Puede manifestarse un eritema o leve hinchazón perifolicular transitorio que remite habitualmente en pocas horas.
							</li>
						</ul>
					</div>

					<!-- 3. AUTORIZACIÓN DE MENORES CON ZONA DE CARGA DE ARCHIVO -->
					<div class="p-4 bg-bg-muted/20 border border-border-subtle rounded-2xl space-y-4">
						...
						<div v-if="isMinor" class="space-y-4 pt-2 border-t border-border-subtle">
							<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
								<div class="space-y-1">
									...
									<input
										...
										type="text"
										placeholder="Nombre completo del progenitor"
							...
							<AuthorizationUploader
								...
								title="Adjuntar Documento Firmado de Autorización de Menor"
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
						<div class="space-y-1.5">
							<label class="text-[11px] font-bold text-text-primary uppercase block">Profesional Aplicador/a</label>
							<input
